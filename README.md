...
- 名前未入力・希望未選択を画面内メッセージで案内
- 入力状況に応じて「次へ」「確認する」を有効化
- Enterキーによる名前入力・管理者ログイン操作
- 画面移動時のフォーカス調整
- 完了画面に「次の方へ」ボタンを追加
- 3秒後の自動リセットは維持
- キーボード操作、動きを減らす設定、強制カラーモードへ配慮
- 保存・イベント管理・CSV・バックアップなど既存機能は維持



## Version 2.3 “Admin Dashboard”

管理者向け画面を、参加者画面と同じデザインシステムへ統一しました。

- 管理者ログイン画面
- 現在のイベントと回答数を示す概要カード
- イベント管理とデータ管理を整理
- 回答一覧をカード形式へ変更
- PCでは2列、スマートフォンでは1列のレスポンシブ表示
- イベント、CSV、バックアップ、編集、削除など既存機能を維持
